...
function crawlFinding({ sourceUrl, targetUrl, resource, anchor }) {
  ...
  if (permanent && status >= 200 && status < 300) {
    const preparedRepair = {
      ...
      before: target,
      after: finalUrl,
      instruction: `Replace the link target ${target} with its permanent destination ${finalUrl}.`,
      verification: [
        `Confirm ${finalUrl} returns a successful response.`,
        `Confirm ${source} links directly to ${finalUrl}.`,
      ],
    };
    return {
