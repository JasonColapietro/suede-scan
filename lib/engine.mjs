...
function crawlFinding({ sourceUrl, targetUrl, resource, anchor, verifiedFallbackUrl }) {
  ...
  if (status === 404 || status === 410) {
    const replacement = cleanEvidenceUrl(verifiedFallbackUrl);
    const preparedRepair = replacement && replacement !== target ? {
      kind: 'replace-link-target',
      ready: true,
      before: target,
      after: replacement,
      instruction: "Replace the confirmed dead link target with the verified audited page; publish only if it matches the anchor's intended destination.",
      verification: [
    ...
    return {
      ...
      action: preparedRepair?.instruction || 'Replace the target with a verified live page or remove the link if no replacement is intended.',
      evidence: { sourceUrl: source, targetUrl: target, finalUrl, status, anchorText: anchor, redirectChain },
      preparedRepair,
    };
  }
...
export async function crawlSiteLinks(rootPage, options = {}) {
  ...
  while (queue.length > 0 && pagesVisited < limits.maxPages && linksChecked < limits.maxLinks && Date.now() < requestBudget.deadline) {
    ...
    for (const link of links) {
      ...
      const finding = unconfirmedDead ? null : crawlFinding({
        sourceUrl: page.url,
        targetUrl: link.target.href,
        resource,
        anchor: link.anchor,
        verifiedFallbackUrl: rootPage.finalUrl,
      });
