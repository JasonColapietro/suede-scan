...
export const PUBLIC_CRAWL_LIMITS = Object.freeze({
  maxPages: 6,
  maxLinks: 30,
  maxRequests: 40,
  maxDepth: 1,
...
async function requestPublicUrl(rawUrl, options = {}) {
  ...
  for (let redirect = 0; redirect <= maxRedirects; redirect += 1) {
    ...
    if (allowedOrigin && current.origin !== allowedOrigin) {
      throw new Error('The link redirected outside the audited site.');
    }
    if (disallowQuery && current.search) {
      throw new Error('Query-bearing links are not crawled.');
    }
    if (requestBudget) {
    ...
    const addresses = await awaitBeforeDeadline(
      assertPublicUrl(current, { lookupImpl }),
      absoluteDeadline,
    );
    if (Date.now() >= absoluteDeadline) throw new Error('The audit reached its response deadline.');
    ...
    if (fetchImpl) {
      ...
      response = await requestImpl(current, { addresses, timeoutMs: effectiveTimeoutMs });
    }
    ...
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          ...
          try { onTimeout?.(); } catch { /* best-effort stream cleanup */ }
        }, remainingMs);
      }),
    ]);
  } finally {
...
function anchorText(tag) {
  const content = tag.match(/<a\b[^>]*>([\s\S]*?)<\/a>/i)?.[1] || '';
  return strip(content).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 160) || 'Unlabelled link';
}
...
function linksFromHtml(html, pageUrl, auditedOrigin) {
  ...
  for (const tag of html.match(/<a\b[^>]*>[\s\S]*?<\/a>/gi) || []) {
    ...
    try {
      ...
      target.hash = '';
      links.push({ target, anchor: anchorText(tag) });
    } catch { /* malformed and unsupported links are not fetched */ }
  }
  return links;
}

function preparedRedirectRepair(sourceUrl, targetUrl, finalUrl, redirects) {
  if (redirects.length === 0 || redirects.some((item) => item.status !== 301 && item.status !== 308)) return null;
  const before = cleanEvidenceUrl(targetUrl);
  const after = cleanEvidenceUrl(finalUrl);
  ...
  return {
    ...
    before,
    after,
    instruction: `Replace the link target ${before} with its permanent destination ${after}.`,
    verification: [
      `Confirm ${after} returns a successful response.`,
      `Confirm ${cleanEvidenceUrl(sourceUrl)} links directly to ${after}.`,
    ],
  };
}
...
function crawlFinding({ sourceUrl, targetUrl, resource, anchor }) {
  ...
  const preparedRepair = preparedRedirectRepair(sourceUrl, targetUrl, resource.url, resource.redirects || []);

  if (preparedRepair && status >= 200 && status < 300) {
    return {
      ...
      severity: 'low',
      observed: `${target} redirects to ${finalUrl}.`,
  ...
  if (status === 404 || status === 410) {
    return {
      ...
      severity: 'high',
      observed: `${target} returned HTTP ${status} from ${source}.`,
...
export async function crawlSiteLinks(rootPage, options = {}) {
  const configured = { ...PUBLIC_CRAWL_LIMITS, ...(options.crawl || options) };
  const limits = Object.fromEntries(Object.entries(PUBLIC_CRAWL_LIMITS).map(([key, fallback]) => {
  ...
  while (queue.length > 0 && pagesVisited < limits.maxPages && linksChecked < limits.maxLinks && Date.now() < requestBudget.deadline) {
    ...
    for (const link of links) {
      ...
      linksChecked += 1;
      const remainingMs = Math.max(250, requestBudget.deadline - Date.now());
      ...
      if (unconfirmedDead || resource.status === 0 || (resource.status >= 400 && resource.status !== 404 && resource.status !== 410 && resource.status < 500)) unknownLinks += 1;
      const finding = unconfirmedDead
        ? null
        : crawlFinding({ sourceUrl: page.url, targetUrl: link.target.href, resource, anchor: link.anchor });
...
function policyForCrawler(page, crawler) {
  ...
  return robots?.ok
    ? crawlerPolicy(robots.text, crawler, path)
    : robots?.status === 404
      ? { state: 'open', detail: 'No robots.txt file was published; no block for the submitted page was detected.' }
...
function platformCheck(page, id, name, crawler) {
  ...
  return makeCheck(
    `crawler-${id}`,
    'AI discovery',
    `${name} crawler can access the submitted page`,
    policy.state === 'open',
...
export function summarize(checks) {
  ...
  const recommendations = checks
    .filter((check) => !check.pass && check.recommend !== false)
    ...
    .map((check) => ({
      ...
      ...(check.preparedRepair ? { preparedRepair: check.preparedRepair } : {}),
    }));
...
export async function runTier(tier, url, options = {}) {
  ...
  const crawl = tier === 'audit' ? await crawlSiteLinks(page, boundedOptions) : null;
  const checks = tier === 'audit' ? [...scanChecks(page), ...auditChecks(page)] : scanChecks(page);
  if (crawl) {
    checks.push(makeCheck(
      'site-links',
      'Site integrity',
      'Checked internal links avoid confirmed dead destinations',
      crawl.linksChecked > 0
        && crawl.unknownLinks === 0
        && !crawl.truncated
        && crawl.brokenLinks === 0
        && crawl.unavailableLinks === 0,
      `${crawl.linksChecked} checked, ${crawl.brokenLinks} broken, ${crawl.unavailableLinks} unavailable, ${crawl.unknownLinks} unknown`,
      'Repair confirmed dead internal destinations and rerun the bounded crawl.',
      'high',
      { recommend: false },
    ));
  }
  ...
  return {
    ...
    methodology: 'Automated inspection of bounded same-origin public HTML and links, robots.txt, llms.txt, and sitemap.xml. Search access uses OAI-SearchBot, Claude-SearchBot, PerplexityBot, and Googlebot; GPTBot, ClaudeBot, and Google-Extended are shown separately as non-scoring model-use controls. This report does not run prompts inside AI answer engines or predict citations.',
    ...summary,
