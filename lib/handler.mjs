...
function cleanOperatorFinding(repair, index, domain) {
  ...
  if (evidence && typeof evidence === 'object' && !Array.isArray(evidence)) {
    ...
    if (sourceUrl && targetUrl && finalUrl && Number.isInteger(status) && status >= 0 && status <= 599) {
      const subtype = ['broken-link', 'redirect-link', 'unavailable-link'].includes(repair.kind) ? repair.kind : null;
...
export function buildOperatorHandoff(result) {
  ...
  while (findings.length > 0) {
    ...
    if (Buffer.from(JSON.stringify(handoff), 'utf8').toString('base64url').length <= HANDOFF_MAX_ENCODED) return handoff;
